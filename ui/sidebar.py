...
def render_sidebar(
    lang: Dict, samples: List[Dict], methods: List[Dict], has_masks: bool
) -> Dict:
    """
    渲染侧边栏并返回用户选择的配置
    ...
    with st.sidebar:
        ...
        # 回调函数
        def go_prev():
            st.session_state.selected_sample_idx = max(
                0, st.session_state.selected_sample_idx - 1
            )

        def go_next():
            st.session_state.selected_sample_idx = min(
                max_start_idx, st.session_state.selected_sample_idx + 1
            )
        ...
        if num_rows == 1:
            st.caption(
                f"{lang['current_label']}: {sample_names[st.session_state.selected_sample_idx]} ({st.session_state.selected_sample_idx + 1}/{len(samples)})"
            )
        else:
            st.caption(
                f"{lang['range_label']}: {st.session_state.selected_sample_idx + 1}-{end_idx} / {len(samples)}"
            )
        ...
        with st.expander(lang["instructions"], expanded=False):
            ...
            **Folder List Mode:**
            1. Enter folder paths (one per line)
            2. Each folder represents a method
            3. System automatically scans and matches images
            4. Check loading summary for statistics
                """)
